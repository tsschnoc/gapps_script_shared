...
//    var queryString = "FIND {" + number.formatPhoneForSearch() + "*} IN Name Fields returning account(id, phone, name), contact(name, id, phone, MobilePhone, HomePhone, OtherPhone, Weiteres_Telefon_direkt__c, firstname, lastname), lead(name, id, phone, firstname, lastname), Zugangsdaten__c(name, id, Type__c, Password__c, Token__c)"    ;
//    var queryString = "FIND {" + number.formatPhoneForSearch() + "*} returning account(id, phone, name), contact(name, id, account.Name, phone, MobilePhone, HomePhone, OtherPhone, Weiteres_Telefon_direkt__c, Email, Private_Email__c, Weitere_Email__c, firstname, lastname), lead(name, id, phone, firstname, lastname), Zugangsdaten__c(name, id, Type__c, Password__c, Token__c)";    
...
//                  var number = $(this).parent().text().trim();
  ...
  //                  var url = 'http://10.71.115.221/command.htm?number=' + escape(number).replace('+', '%2B');
          //          var url = 'http://www.schnocklake.de?number=' + escape(number).replace('+', '%2B');
                    ...
                    console.log(url);
                    //$('#callToFrame').attr("src",url); 
                    window.open("http://rawgithub.com/tsschnoc/gapps_script_shared/master/pusher/calltopopup.html?url=" + encodeURIComponent(url), "fenster1", "width=600,height=400,status=yes,scrollbars=yes,resizable=yes");
                  }
                });
            console.log( $(this));
